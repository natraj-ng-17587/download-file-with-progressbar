...
function download(url, filepath, onError, onDone, onProgress,headers) {
    let out = fs.createWriteStream(filepath);
    const options = {
        headers:headers,   // Bypasses the certificate validation (use with caution)
        rejectUnauthorized: false  // Bypasses the certificate validation (use with caution)

      };
      
    var req = request.get(url,options);
    var isAbort = false;
...
module.exports = function(url, option = {}) {
    ...
    return download(url, filepath, onError, onDone, onProgress,headers);
};
